...
public:
    ...
    void Update(float deltaTime, const std::shared_ptr<MapInfoLoader>& mapLoader, const glm::vec2& playerPos = {0, 0});
...
private:
    ...
    glm::vec2 m_PlayerPos = {0.0f, 0.0f};   // 玩家位置
    float m_CurrentSpeed = 0.0f;            // 当前实际速度
    float m_MaxSpeed = 0.0f;                // 最大速度
    float m_Acceleration = 0.05f;           // 加速度
    bool m_HitWall = false;                 // 是否撞墙
    float m_RecoveryTimer = 0.0f;           // 撞墙后恢复计时器
    float m_TurnDelayTimer = 0.0f;       // 转向延迟计时器
    float m_DirectionChangeDelay = 0.3f; // 方向改变所需的延迟时间(秒)
    Character::direction m_TargetDirection = Character::direction::RIGHT; // 目标方向
};
